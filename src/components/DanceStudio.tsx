...
export function DanceStudio({ song, onChange, playPosition, beatsPerLoop }: DanceStudioProps) {
  ...
  useEffect(() => {
    liveRef.current = { playPosition, secondsPerBeat, bounce: dance.bounce };
  }, [playPosition, secondsPerBeat, dance.bounce]);
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const totalBeats = beatsPerLoop * Math.max(1, Math.floor(song.repeats));
